...
@app.route("/removeplan2")
def remove_specific_plans2():
    global current_plan_index
...
def set_plan2():
    ...
    for i, plan in enumerate(plans):
        if plan.lower() == plan_to_set:
            ...
            save_data()
            return f"'{plans[i]}' En pantalla nephuo7"
    return "Plan no encontrado nephtuniethink"
...
@app.route("/addcall2", methods=['GET'])
def add_call2():
    entries = request.args.get("entries", "").split()
    if len(entries) % 2 != 0:
...
@app.route("/removecall2", methods=['GET'])
def remove_call2():
    names = request.args.get("entries", "").split()
    removed = []
    names = [name.lower() for name in names]
...
@app.route("/resetcall2", methods=['GET'])
def reset_call2():
    call_participants.clear()
    save_data()
